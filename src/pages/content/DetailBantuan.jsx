...
function DetailBantuan({ customer }) {
  const [editorData, setEditorData] = useState('');
  const [editorDataEng, setEditorDataEng] = useState('');
  const [customerData, setCustomerData] = useState(customer);

  useEffect(() => {
    if (customer) {
      setEditorData(customer.article_content);
      setEditorDataEng(customer.article_content_en);
              ...
              <div className='mt-5'>
                <label style={{ fontWeight: "bold", marginTop: '40px', marginRight: '10px' }} className="font-plus-jakarta-sans s">Title</label>
                  ...
                  <Input
                      ...
                      name="nama_bahan"
                      value={customer.article_title}
                  />
                <label style={{ fontWeight: "bold", marginTop: '40px', marginRight: '10px' }} className="font-plus-jakarta-sans s">Title in Eng</label>
                  {/* Menghubungkan input tarif dengan state tarif */}
                  <Input
                      type="text"
                      className="mt-2 mb-2 h-14"
                      name="nama_bahan"
                      value={customer.article_title_en}
                  />
              </div>
              <br />
                <div>
                <i> slug: {customer.article_slug} </i>
      ...
      <div className="mt-12 mb-8 ">
                  ...
                  <CardBody>
                    <div className='w-full flex space-x-2'>
                      <div className='w-1/2'>
                      <label className='font-bold'>Categori : </label>
                      <Input className='w-full mt-2 h-10 mb-2' value={customer.category}/>
                      </div>
                      <div className='w-1/2'>
                      <label className='font-bold'>Categori : </label>
                      <Input className='w-full mt-2 h-10 mb-2' value={customer.category_en}/>
                      </div>
                    </div>
                    <label label style={{ fontWeight: "bold", marginTop: '20px', marginRight: '10px' }} className="font-plus-jakarta-sans font-bold">Daftar Topics :</label>
                                <Select
                                    className="w-full mt-2 h-12" // Contoh penggunaan kelas Tailwind untuk mengatur lebar
                                    placeholder="Pilih Negara"
                                    value={customer.dataTopic}
                                >
