    ...
    const [keyword, setKeyword] = useState('');
     ...
     const GetDataBlog = async () => {
        try {
            ...
            const response = await axios.get(`${Baseurl}backend/master/data-blog?page=${currentPage}&limit=${limit}&keyword=${keyword}`, {
                headers: {
                ...
                <table className="w-full table-auto">
                    <thead>
                        <tr>
                            ...
                            <th className="border px-4 py-2">Title Slug</th>
                            <th className="border px-4 py-2">Kategori</th>
                            <th className="border px-4 py-2">Total Dibaca</th>
                            <th className="border px-4 py-2">Author</th>
                            <th className="border px-4 py-2">Tanggal Pembuatan</th>
                    ...
                    <tbody>
                        {DataBlog.map((item, index) => (
                            <tr key={item.id}> {/* Pastikan item.id atau id yang unik digunakan sebagai key */}
                                ...
                                <td className="border px-4 py-2">{item.title_slug}</td>
                                <td className="border px-4 py-2">{item.kategori}</td>
                                <td className="border px-4 py-2 text-center">{item.dibaca}</td>
