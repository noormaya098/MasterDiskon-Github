    ...
    const [keyword, setKeyword] = useState('');
     ...
     const GetDataBlogKategori = async () => {
        try {
            ...
            const response = await axios.get(`${Baseurl}backend/master/data-blog-kategori?page=${currentPage}&limit=${limit}&keyword=${keyword}`, {
                headers: {
                ...
                <table className="w-full table-auto">
                    <thead>
                        <tr>
                            ...
                            <th className="border px-4 py-2">Nama Blog Kategori</th>
                            <th className="border px-4 py-2">Nama Blog Kategori en</th>
                            <th className="border px-4 py-2">Slug Blog Kategori</th>
                    ...
                    <tbody>
                        {DataBlogKategori.map((item, index) => (
                            <tr key={item.id}> {/* Pastikan item.id atau id yang unik digunakan sebagai key */}
                                ...
                                <td className="border px-4 py-2">{item.name_blog_category}</td>
                                <td className="border px-4 py-2">{item.name_blog_category_en}</td>
                                <td className="border px-4 py-2">{item.slug_blog_category}</td>
